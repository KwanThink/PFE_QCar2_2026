...
# Compute tracking errors and basic solver statistics.
def compute_tracking_metrics(states: np.ndarray, reference_states: np.ndarray, controls: np.ndarray, statuses: np.ndarray) -> dict:
    ...
    metrics = {
        ...
        "max_abs_delta": float(np.max(np.abs(controls[:, 0]))) if len(controls) else 0.0,
        "max_abs_ax": float(np.max(np.abs(controls[:, 1]))) if len(controls) else 0.0,
    }
    return metrics
