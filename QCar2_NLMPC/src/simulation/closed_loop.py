...
from acados_interface.sim_builder import create_sim_solver
from model_traj.config import path_from_config
from model_traj.qcar2_nlmpc_controller import QCar2NLMPC
from model_traj.reference_loader import ReferenceTrajectory
from simulation.metrics import compute_tracking_metrics, save_metrics
from simulation.plots import save_all_plots
...
# The run prefix is read from the config file.
...
# Choose the initial simulation state.
# It can come from the reference or from the config override.
...
# Choose the initial solver input guess.
# It can come from the reference or from the config override.
...
# Propagate the QCar2 model by one sampling step.
# The simulator returns the next state and integration status.
...
# Run the full closed-loop NMPC simulation.
# Results, metrics, and plots are saved in a timestamped folder.
def run_closed_loop(cfg: dict, reference: ReferenceTrajectory, plot: bool = True) -> ClosedLoopResult:
    N = int(cfg["mpc"]["N"])
    Ts = float(cfg["qcar2"]["Ts"])
    ...
    np.savetxt(
        ...
        delimiter=",",
        header="t,x_ref,y_ref,theta_ref,varphi_ref",
        comments="",
    )
    np.savetxt(
