...
# Plot the driven trajectory against the reference path.
# This helps evaluate XY tracking quality visually.
def plot_tracking(states: np.ndarray, reference_states: np.ndarray, output_path: str | Path) -> None:
    ...
    plt.xlabel("x [m]")
    plt.ylabel("y [m]")
    plt.title("QCar2 trajectory tracking")
    plt.legend()
    plt.tight_layout()
...
# Plot simulated states and their references over time.
# Each state component is shown in a separate figure panel.
def plot_states(time: np.ndarray, states: np.ndarray, reference_states: np.ndarray, output_path: str | Path) -> None:
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    labels = ["x [m]", "y [m]", "theta [rad]", "phi [rad]"]
...
# Plot applied controls and reference controls over time.
# This highlights saturation or strong control deviations.
def plot_controls(time: np.ndarray, controls: np.ndarray, reference_controls: np.ndarray, output_path: str | Path) -> None:
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    labels = ["v [m/s]", "omega_s [rad/s]"]
...
# Plot NMPC solver time at each control step.
# It helps verify real-time feasibility.
...
# Generate and save all standard closed-loop diagnostic plots.
# This is called once after a simulation run finishes.
def save_all_plots(run_dir: str | Path, time_states: np.ndarray, states: np.ndarray, reference_states: np.ndarray,
                   time_controls: np.ndarray, controls: np.ndarray, reference_controls: np.ndarray,
                   solve_times: np.ndarray) -> None:
