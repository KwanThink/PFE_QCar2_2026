...
# Build a diagonal numpy matrix from a list of weights.
# This keeps cost matrix creation compact and readable.
def _diag(values: list[float] | tuple[float, ...]) -> np.ndarray:
    return np.diag(np.asarray(values, dtype=float))


# Configure the acados optimal control problem.
# Model, costs, constraints, and solver options are defined here.
def build_ocp(cfg: dict):
    from acados_template import AcadosOcp

    L = float(cfg["qcar2"]["L"])
    N = int(cfg["mpc"]["N"])
    tf = float(cfg["mpc"].get("tf", N * float(cfg["qcar2"]["Ts"])))

    model = export_qcar2_model(L=L)
    ocp = AcadosOcp()
    ocp.model = model
    ...
    ocp.constraints.lbu = np.array([constraints["v_min"], constraints["omega_s_min"]], dtype=float)
    ocp.constraints.ubu = np.array([constraints["v_max"], constraints["omega_s_max"]], dtype=float)
    ocp.constraints.idxbu = np.array([0, 1], dtype=int)

    ocp.constraints.idxbx = np.array([3], dtype=int)
    ocp.constraints.lbx = np.array([constraints["phi_min"]], dtype=float)
    ocp.constraints.ubx = np.array([constraints["phi_max"]], dtype=float)
...
# Create an acados OCP solver from the prepared problem.
...
def create_ocp_solver(cfg: dict, generate: bool = True, build: bool = True):
    ...
    json_file = path_from_config(cfg, "generated_ocp_dir") / cfg.get("acados", {}).get(
        "ocp_json_file", "acados_ocp_qcar2.json"
    )
    return AcadosOcpSolver(ocp, json_file=str(json_file), generate=generate, build=build)


# Build and return the default acados OCP solver.
# This wrapper is used by command-line scripts.
